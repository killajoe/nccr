...
char totalstr[32],pricestr[8],cashstr[8];
...
char **desc;
float *price;
float total=0.,totaltax1=0.,totaltax2=0.,bar=0.;
...
int main(void) {
	...
	depo = (char **)malloc(lines * sizeof(char *));
	typ  = (char **)malloc(lines * sizeof(char *));
	tax  = (char **)malloc(lines * sizeof(char *));
	...
	for(j=0; j<lines; j++) {
		...
		token = strtok(NULL, csv);
		depo[j] = (char *)malloc((strlen(token) + 1) * sizeof(char));
		strcpy(depo[j], token);
		
		token = strtok(NULL, csv);
		tax[j] = (char *)malloc((strlen(token) + 1) * sizeof(char));
		strcpy(tax[j], token);
	...
	strncpy(desc[0], arrow, 3);
	strncpy(desc[1], arrow, 3);
	strncpy(desc[2], arrow, 3);
			...
			case KeyPress:
				...
				switch(keysym) {
					...
					case XK_Home:
						...
						len = 0;
						string[0] = '\0';
						sprintf(totalstr, "%.2f (MwSt %.2f)", total, totaltax1+totaltax2);
					...
					case XK_Linefeed:
						...
						if(bill[0][cartno-1] == j) {
							...
							XFillRectangle(dpy, win, gchead, 5, 5, attributes.width-10, HEAD);
							total += price[j];
							if(strcmp("TX1", tax[j]) == 0)
								totaltax1 += TAX1 * price[j];
							else
								totaltax2 += TAX2 * price[j];
							...
							switch(*(*(depo+j)+2)) {
								...
								case '3':
									...
									break;
								default:
									fprintf(stderr, "unknown deposit value! something is wrong ...\n");
									fflush(stderr);
									break;
							}
							sprintf(totalstr, "%.2f", total);
...
void printbill() {
	...
	/* start printing */
	write(fd, "R\x5d\x7d", strlen("R\x5d\x7d"));
	write(fd, "bezahl Naturkost\n", strlen("bezahl Naturkost\n"));
	write(fd, buffer, strlen(buffer));

	for(i=0; i<cartno; i++) {
		m = 0;
		for(k=0; k<BWIDTH-9; k++) {
			bufdesc[k] = desc[bill[0][i]][k+m];
			if(desc[bill[0][i]][k+m] == ae[0] && desc[bill[0][i]][k+m+1] == ae[1]) {
				bufdesc[k]  = xae[0];
				m++;
			}
			if(desc[bill[0][i]][k+m] == oe[0] && desc[bill[0][i]][k+m+1] == oe[1]) {
				bufdesc[k]  = xoe[0];
				m++;
			}
			if(desc[bill[0][i]][k+m] == ue[0] && desc[bill[0][i]][k+m+1] == ue[1]) {
				bufdesc[k]  = xue[0];
				m++;
			}
			if(desc[bill[0][i]][k+m] == sz[0] && desc[bill[0][i]][k+m+1] == sz[1]) {
				bufdesc[k]  = xsz[0];
				m++;
			}
			if(desc[bill[0][i]][k] == '\0') {
				for(; k<BWIDTH-9; k++)
					bufdesc[k] = ' ';
				break;
			}
		}
		if(price[bill[0][i]] > 9.99)
			bufdesc[k] = '\0';
		else {
			bufdesc[k] = ' ';
			bufdesc[k+1] = '\0';
		}
		if(strcmp("TX1", tax[bill[0][i]]) == 0)
			txtyp = 'A';
		else
			txtyp = 'B';
		sprintf(buffer, "%s  %.2f %c\n", bufdesc, price[bill[0][i]], txtyp);
		write(fd,buffer,strlen(buffer));
		usleep(300000);
	}
